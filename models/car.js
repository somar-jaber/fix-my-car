...
function validateCar(reqBody) {
    const schema = Joi.object({
        ...
        branches: Joi.array().min(0).items(Joi.string().allow("", null)),  // hte min(0) method will allow empty arrays. if the car gets into a new branch that branch will be pushed to the 'branches' array in the car doc. And the server will set the branch depending on the token that identify the 'users' who are using the system  
