...
router.get("/", authMiddleware.authMiddleware, asyncTryCatch(async(req, res) => {
    ...
    cars = await CarModel.populate(cars , {
        path: 'repairs',
        model: 'Repair', // if the model is not in the same file we need to mention it in the "model" filed
    });
...
router.delete("/:id", authMiddleware.authMiddleware, asyncTryCatch(async(req, res) => {
    ...
    if (carSample.repairs.length > 0)  return res.status(400).send("400 Bad Request: Car can't be deleted because it has Repairs releated to it.");

    await carSample.deleteOne();
    return res.send(carSample);
}));
