...
router.get("/", authMiddleware, asyncTryCath(async(req, res) => {
    ...
    repairs = await RepairModel.populate(repairs, {
        path: "car",
        model: "Car", // if the model is not in the same file we need to mention it in the "model" filed
    });

    repairs = await RepairModel.populate(repairs, {
        path: "workers",
        model: "Worker", // if the model is not in the same file we need to mention it in the "model" filed
...
router.post("/", authMiddleware, async(req, res) => {
    ...
    */
    ...
    let workerSamples;
    try {
        workerSamples = await findMultiDocs(WorkerModel, repairSample.workers);
        // the updating for workers docs will happend in the try catch
    }
    catch(ex) {
